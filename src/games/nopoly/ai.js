 ...
 *
 * Difficulties differ only in how deep they look (and how much randomness sits
 * on top), so the same evaluation drives all of them.
 */
export const DIFFICULTIES = {
  easy: { id: 'easy', name: 'Easy', depth: 1, jitter: 90, blurb: 'Looks one move ahead.' },
  normal: { id: 'normal', name: 'Normal', depth: 3, jitter: 25, blurb: 'Looks three moves ahead.' },
  hard: { id: 'hard', name: 'Hard', depth: 4, jitter: 0, blurb: 'Looks four moves ahead and takes no chances.' },
};
...
export function evaluate(state, side) {
  ...
  for (const owner of [side, opponentOf(side)]) {
    ...
    for (const piece of piecesOf(state, owner)) {
      ...
      value += advancement(owner, piece.index) * (piece.type === 'farmer' ? 6 : 2);
...
export function chooseMove(state, difficulty = 'normal', random = Math.random) {
  ...
  let alpha = -Infinity;
  for (const move of moves) {
    const raw = search(applyMove(state, move), level.depth - 1, alpha, Infinity, side);
    const score = raw + (level.jitter ? (random() - 0.5) * level.jitter : 0);
    if (score > bestScore) {
      bestScore = score;
      best = move;
